...
static int tmc524x_stepper_driver_set_event_cb(const struct device *stepper,
						     stepper_event_cb_t callback, void *user_data)
{
	struct tmc524x_stepper_driver_data *data = stepper->data;
...
{
	const struct tmc524x_stepper_driver_config *config = dev->config;
	...
	err = tmc524x_read(controller, TMC524X_CHOPCONF, &reg_value);
	if (err != 0) {
		return -EIO;
	}
	...
	return tmc524x_write(controller, TMC524X_CHOPCONF, reg_value);
}
						     ...
						     enum stepper_micro_step_resolution res)
{
	...
	err = tmc524x_write(controller, TMC524X_CHOPCONF, reg_value);
	if (err != 0) {
		return -EIO;
	}
...
static int tmc524x_stepper_driver_init(const struct device *dev)
{
	...
	err = tmc524x_write(controller, TMC524X_COOLCONF,
			    stall_guard_threshold << TMC5XXX_COOLCONF_SG2_THRESHOLD_VALUE_SHIFT);
	if (err != 0) {
		return -EIO;
	}

	err = tmc524x_stepper_driver_set_micro_step_res(dev, config->default_micro_step_res);
	if (err != 0) {
		return -EIO;
	}
