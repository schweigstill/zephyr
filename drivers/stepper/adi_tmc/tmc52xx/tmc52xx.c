...
static void rampstat_work_handler(struct k_work *work)
{
	...
	if (ramp_stat_values > 0) {
...
#ifdef CONFIG_STEPPER_ADI_TMC52XX_STEPPER
		case TMC5XXX_STOP_LEFT_EVENT:
			LOG_DBG("RAMPSTAT %s:Left end-stop detected", dev->name);
			tmc52xx_stepper_trigger_cb(motion_controller,
		...
		case TMC5XXX_POS_REACHED_AND_EVENT:
			...
			break;
#endif /* CONFIG_STEPPER_ADI_TMC52XX_STEPPER */
#ifdef CONFIG_STEPPER_ADI_TMC52XX_STEPPER_DRV
		case TMC5XXX_STOP_SG_EVENT:
			LOG_DBG("RAMPSTAT %s:Stall detected", dev->name);
			tmc52xx_stepper_stallguard_enable(dev, false);
			tmc52xx_stepper_drv_trigger_cb(stepper_driver,
				STEPPER_DRV_EVENT_STALL_DETECTED);
			break;
#endif /* CONFIG_STEPPER_ADI_TMC52XX_STEPPER_DRV */
		default:
	...
	static const struct tmc52xx_config tmc52xx_config_##inst = {COND_CODE_1			   \
		...
		(TMC52XX_CONFIG_UART(inst))),                                                      \
		 ...
		 .motion_controller = DEVICE_DT_GET_OR_NULL(DT_CHILD_BY_COMPATIBLE(                \
			 DT_DRV_INST(inst), adi_tmc52xx_stepper)),                                 \
		 .stepper_driver = DEVICE_DT_GET_OR_NULL(                                          \
			 DT_CHILD_BY_COMPATIBLE(DT_DRV_INST(inst), adi_tmc52xx_stepper_drv))};     \
